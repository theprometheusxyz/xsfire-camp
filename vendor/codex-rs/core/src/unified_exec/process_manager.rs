...
impl UnifiedExecProcessManager {
    ...
    pub(crate) async fn exec_command(
        ...
        let request_process_id = request.process_id.clone();
        if let Some(delegate) = &self.delegate {
            let response = delegate.exec_command(request, context).await;
            if response.is_err()
                || response
                    .as_ref()
                    .ok()
                    .is_some_and(|response| response.process_id.is_none())
            {
                self.release_process_id(&request_process_id).await;
            }
            return response;
        }
    ...
    pub(crate) async fn write_stdin(
        ...
        let PreparedProcessHandles {
            ...
            cwd,
            tty,
        } = match self.prepare_process_handles(process_id.as_str()).await {
            Ok(handles) => handles,
            Err(UnifiedExecError::UnknownProcessId { .. }) if self.delegate.is_some() => {
                return self
                    .delegate
                    .as_ref()
                    .expect("delegate presence checked above")
                    .write_stdin(request, context)
                    .await;
            }
    ...
    pub(crate) async fn terminate_all_processes(&self) {
        ...
        if let Some(delegate) = &self.delegate {
            let _ = delegate.terminate_all_processes().await;
        }
    }
}
